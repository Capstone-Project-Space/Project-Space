...
private:
	...
	enum class Side { NONE, LEFT, RIGHT, CENTERX, TOP, BOTTOM, CENTERY, INFRONT };
	Side leftSide, topSide, rightSide, bottomSide;

	static Side FromSide(const Side target, const std::string_view& side);
	...
	static float GetValue(const std::pair<float, float> targetX, const std::variant<Literal, Percentage>& position, std::shared_ptr<Window> window, const Side side);
};
